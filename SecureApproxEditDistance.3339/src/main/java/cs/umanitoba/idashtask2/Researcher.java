...
public class Researcher {
    ...
    public static void main(String[] args) throws ClassNotFoundException, InstantiationException, IllegalAccessException, FileNotFoundException, IOException {
        ...
        final int bandWidth = Integer.parseInt(args[3]);
...
//                    System.out.println(queryCopies.size() + " " + queryIndex);
                    if (queryIndex != queryCopies.size()) {
                        ...
                        run.setParameter(Mode.OPT, port, bandWidth, Arrays.copyOfRange(args, 1, args.length), host, queryCopies, queryIndex);
...
//                    }
        ...
        run.setParameter(Mode.OPT, port, bandWidth, Arrays.copyOfRange(args, 1, args.length),host, queryCopies, 0);//removing the classname
//        run.run();
